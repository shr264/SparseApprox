...
pdf(paste(toString(N),'cvgc-cn.pdf',sep=''))
...
pdf(paste(toString(N),'cvgc2-cn.pdf',sep=''))
plot(ista2a$error2/sum(betastar^2),ylab='Relative Norm Error',xlab = 'Iteration',type = 'l', col = '1', ylim = c(0.00,0.0065))
...
op <- microbenchmark(
    ...
    times=10L)
print(op)
pdf(paste(toString(N),'timing-cn.pdf',sep=''))
boxplot(op)
dev.off()

difftable <- rbind(subg2$diff,ista2a$diff,fista2a$diff,admm2a$diff)/sum(betastar^2)
row.names(difftable) <- c( 'SG','ISTA', 'FISTA','ADMM')
write.table(difftable,file=paste(toString(N),'relativenormerror-cn.txt',sep=''),row.names = TRUE, col.names = FALSE) 
}
